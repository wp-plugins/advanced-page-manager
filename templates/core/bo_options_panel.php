<div class="wrap">
	...
	<?php $base_url = ApmOptions::get_base_url() ?>

	<?php 
		$feedback_msg = !empty($_GET['apm_options_msg']) ? $_GET['apm_options_msg'] : ApmOptions::$feedback['msg'];
		$feedback_type = !empty($_GET['apm_options_msg_type']) ? $_GET['apm_options_msg_type'] : ApmOptions::$feedback['type'];
	?>
	...
	<div class="metabox-holder">
		<div class="postbox">
			<h3 class="hndle"><span><?php _e('Plugin options') ?></span></h3>
			<div class="inside">
							...
							<?php else: ?>
								<?php foreach($lost_pages as $page): ?>
									<tr>
										...
										<td><a href="<?php echo add_query_arg(array('apm_options_action'=>'restore_page','wp_id'=>$page->ID),$base_url) ?>"><?php _e("Restore at the end of the page tree", ApmConfig::i18n_domain) ?></a></td>
									</tr>
								<?php endforeach ?>
							<?php endif ?>
						</table>
			...
			<div class="inside">
				<form action="#" method="post">
					<div class="form-table-wrapper">
						<table class="form-table">
							<tr>
								<td><a href="<?php echo add_query_arg(array('apm_options_action'=>'reset_tree_from_wp_pages'),$base_url) ?>"><?php _e("Reinitialize the tree from Wordpress pages", ApmConfig::i18n_domain) ?></a></td>
								<td></td>
							</tr>
							<tr>
								<td><a href="<?php echo add_query_arg(array('apm_options_action'=>'apm_tree_to_wp_pages_tree'),$base_url) ?>"><?php _e('Synchronize Wordpress pages tree with the plugin tree', ApmConfig::i18n_domain) ?></a></td>
								<td></td>
							</tr>
							<tr>
								<td><a href="<?php echo add_query_arg(array('apm_options_action'=>'delete_all_data'),$base_url) ?>"><?php _e('Delete plugin data except options', ApmConfig::i18n_domain) ?></a> : <?php _e('delete plugin tree and fold / unfold data. Doesnt delete plugin options.', ApmConfig::i18n_domain) ?></td>
								<td></td>
							</tr>
							<tr>
								<td><a href="<?php echo add_query_arg(array('apm_options_action'=>'delete_options'),$base_url) ?>"><?php _e('Delete plugin options', ApmConfig::i18n_domain) ?></a> : <?php _e('reinitialize plugin options.', ApmConfig::i18n_domain) ?></td>
								<td></td>
							</tr>
							<tr>
								<td><a href="<?php echo add_query_arg(array('apm_options_action'=>'delete_folding_infos'),$base_url) ?>"><?php _e('Delete fold / unfold data', ApmConfig::i18n_domain) ?></a> : <?php _e('fold all pages.', ApmConfig::i18n_domain) ?></td>
								<td></td>
							</tr>
							<?php do_action('apm_bo_option_panel_add_tr',$base_url) ?>
						</table>
					</div>
				</form>
			</div>
...
<?php /* For debug :
	...
	<div class="metabox-holder">
		<div class="postbox">
			<h3 class="hndle"><span><?php _e('Données de débug') ?></span></h3>
			<div class="inside">
<pre>
